...
export default function Home() {
  ...
  // ── WORLD-CLASS WHITE PROFESSIONAL PDF REPORT ─────────────────────────────
  const genPDF = () => {
    const doc = new jsPDF({orientation:"portrait",unit:"mm",format:"a4"});
    const W=210, M=20;
    let y=0;
    ...
    const paragraph=(lines:string[],x:number,ey:number,sz:number,col:[number,number,number],lh=5.5):number=>{
      doc.setFontSize(sz);setTxt(col);doc.setFont("helvetica","normal");
      lines.forEach(ln=>{doc.text(ln,x,ey);ey+=lh;});
      return ey;
    };

    const wrapText=(t:string,maxW:number,sz:number):string[]=>{
      doc.setFontSize(sz);
    ...
    doc.text("SCIENTIFIC INTEGRITY ANALYSIS PLATFORM  v2.0.0",M+26,11);
    ...
    doc.text("Automated 20-Dimension Integrity Analysis",M,y); y+=16;
    ...
    doc.text(`SciPeerAI v2.0.0  ·  ${activeR.length} Modules  ·  ${totF} Anomalies Flagged`,W-M,9.5,{align:"right"});
    y=22;
    ...
    activeR.forEach((r,i)=>{
      ...
      if(y+cardH>280){
        ...
        doc.setFontSize(7); setTxt([160,175,200]);
        doc.text(`SciPeerAI v2.0.0`,W-M,9.5,{align:"right"});
        y=22;
      }
    ...
    // ────────────────────────────────────────────────────────────────────────
    // FINAL PAGE — EXECUTIVE RECOMMENDATIONS
    // ────────────────────────────────────────────────────────────────────────
    doc.addPage();
    setFill(white); doc.rect(0,0,W,297,"F");
    setFill(navy); doc.rect(0,0,W,14,"F");
    ...
    for(let p=1;p<=pages;p++){
      ...
      doc.text("SciPeerAI  ·  Engineered by Sameer Nadeem  ·  v2.0.0  ·  20 Modules  ·  180 Tests  ·  Bahawalpur, Pakistan",M,291);
    ...
    notify("✓ Professional report downloaded.");
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#060a10",color:"#fff",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:780,margin:"0 auto",padding:"0 24px 100px"}}>
        ...
        {/* ── OVERALL SCORE — NEXUS ELITE ── */}
        {activeDone&&activeR.length>0&&(
          <div style={{background:"rgba(7,11,18,0.98)",border:`1px solid ${avCol}18`,borderRadius:12,padding:"28px 28px 22px",marginBottom:12,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{paddingTop:14,borderTop:"1px solid rgba(255,255,255,0.03)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <span style={{fontSize:8,color:"rgba(255,255,255,0.12)",fontFamily:"Space Mono, monospace",letterSpacing:"0.12em"}}>
                {totF} ANOMALIES · {activeR.length} DIMENSIONS · SciPeerAI v2.0.0
              </span>
